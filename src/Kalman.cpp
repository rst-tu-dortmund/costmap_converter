...
TKalmanFilter::TKalmanFilter(Point_t pt, track_t dt, track_t Accel_noise_mag)
{
  ...
  // Nur x und y Koordinaten messbar!
  kalman->measurementMatrix = (cv::Mat_<track_t>(2, 4) << 1, 0, 0, 0, 0, 1, 0, 0);
  ...
  cv::setIdentity(kalman->measurementNoiseCov, cv::Scalar::all(1e-9));
...
void TKalmanFilter::Prediction()
{
  ...
  LastPosition = Point_t(prediction.at<track_t>(0), prediction.at<track_t>(1));
  LastVelocity = Point_t(prediction.at<track_t>(2), prediction.at<track_t>(3));
}
...
Point_t TKalmanFilter::Update(Point_t p, bool DataCorrect)
{
  ...
  LastVelocity.x = estimated.at<track_t>(2);
  LastVelocity.y = estimated.at<track_t>(3);
  return LastPosition;
}
//---------------------------------------------------------------------------
