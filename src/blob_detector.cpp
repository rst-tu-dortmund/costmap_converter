...
BlobDetector::BlobDetector(const SimpleBlobDetector::Params &parameters) :
  params(parameters)
{
...
const std::vector < std::vector<cv::Point> > BlobDetector::getContours()
{
    return _contours;
}
...
void BlobDetector::findBlobs(const cv::Mat &image, const cv::Mat &binaryImage,
                                   std::vector<Center> &centers, std::vector < std::vector<cv::Point> >&curContours) const
{
    (void)image;
    centers.clear();
    curContours.clear();
